...
def extract_masked(image,linedesc,pad=5,expand=0):
    ...
    #print shape(image), y0-pad,x0-pad,y1+pad,x1+pad
    # line = where(mask,line,amax(line))
    # This If Condition added by Mohsin with Ajraf and Saqib for getting exact value what image shape has
    if size(line) != size(mask):
        line = image
    else:
        line = where(mask,line,amax(line))
    return line
